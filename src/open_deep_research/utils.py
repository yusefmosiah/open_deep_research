...
##########################
# Citation & File Utilities
##########################

def load_validated_citations_from_file(path_str: str) -> list[dict]:
    """Load validated citations from JSON or Markdown file."""
    try:
        ...
        if p.suffix.lower() == ".json":
            ...
            return []
        # assume simple md list format: - [Title](URL)
...
def save_validated_citations(citations: list[dict], base_path: str) -> None:
    """Save validated citations to both JSON and Markdown formats."""
    try:
        from pathlib import Path
        import json

        out_base_path = Path(base_path)
        out_base_path.parent.mkdir(parents=True, exist_ok=True)

        # JSON format
        json_path = out_base_path.with_suffix(".json")
        json_path.write_text(json.dumps(citations, ensure_ascii=False, indent=2), encoding="utf-8")

        # Markdown list format
        md_lines = [
            f"- [{c.get('title') or 'Untitled'}]({c.get('url') or c.get('final_url')})"
            for c in citations
            if (c.get('url') or c.get('final_url'))
        ]
        md_path = out_base_path.with_suffix(".md")
        md_path.write_text("\n".join(md_lines), encoding="utf-8")
    except Exception:
        pass


def convert_inline_links_to_footnotes(text: str, validated_citations: list[dict]) -> str:
    """Convert inline Markdown links to numeric footnotes and append a References section.

    Behavior:
    - Builds a stable mapping of validated citation URLs to footnote numbers (order preserved).
    - Replaces `[anchor](url)` with `anchor[^n]` when the url is in the validated list.
    - Replaces bare occurrences of validated URLs with `[^n]`.
    - Removes any in-text "Validated Sources (allowlist)" blocks and generic "Sources:" trailers.
    - Appends a References section with footnote definitions: `[^n]: Title — URL`.

    If no validated citations are provided, returns the text unchanged.
    """
    try:
        import re
        ...
        def _norm_url(u: str) -> str:
            try:
                parts = urlsplit(u)
                # drop fragment and query for matching, normalize trailing slash
                clean = urlunsplit((parts.scheme.lower(), parts.netloc.lower(), parts.path.rstrip('/'), '', ''))
                return clean
            except Exception:
                return u.rstrip('/').lower()

        # Build ordered map url -> index and keep titles
        url_to_idx: dict[str, int] = {}
        idx_to_entry: list[tuple[str, str]] = []  # (title, url)
        for c in validated_citations:
            url = c.get("url") or c.get("final_url")
            title = c.get("title") or c.get("page_title") or c.get("anchor") or "Untitled"
            if not url:
                continue
            nu = _norm_url(url)
            if nu not in url_to_idx:
                url_to_idx[nu] = len(idx_to_entry) + 1
                idx_to_entry.append((title, url))

        if not url_to_idx:
            return text

        # Remove allowlist and generic sources boilerplate blocks
        # Validated Sources section
        text = re.sub(r"\n+Validated Sources \(allowlist\):[\s\S]*?(?:\n\n|$)", "\n\n", text, flags=re.IGNORECASE)
        # Generic trailing sources line
        text = re.sub(r"\n+\*\*Sources?:\*\*[\s\S]*$", "", text, flags=re.IGNORECASE)

        # Replace inline markdown links that match allowed URLs
        def repl_md_link(m: re.Match) -> str:
            anchor, url = m.group(1), m.group(2)
            nu = _norm_url(url)
            idx = url_to_idx.get(nu)
            if not idx:
                return m.group(0)  # leave untouched if not validated
            # Keep anchor text, drop inline URL, add footnote marker
            return f"{anchor}[^{idx}]"

        text = re.sub(r"\[([^\]]+)\]\((https?://[^)\s]+)\)", repl_md_link, text)

        # Replace bare validated URLs with footnote markers
        ...
        # Append References section with footnote definitions
        lines = ["\n\n---\n\nReferences\n"]
        for i, (title, url) in enumerate(idx_to_entry, start=1):
            lines.append(f"[^{i}]: {title} — {url}")
        references_block = "\n".join(lines)

        # If there's already a References or Footnotes section, append after it; else at end
        return text.rstrip() + references_block
    except Exception:
        return text
...
def strip_model_preamble(text: str) -> str:
    """Remove common model preamble/meta language from generated text."""
    if not text:
        return text

    import re
    lower = text.lower()
    # Heuristic: remove common meta/preamble language at the top
    preamble_markers = [
        "below, i ",
        "compusition guidance",
        "i replace structural scaffolding",
        "i build understanding",
        "i apply",
        "i transform your draft",
    ]

    if any(marker in lower[:600] for marker in preamble_markers):
        # Remove up to first blank-line separation after the marker or a divider line
        parts = re.split(r"\n\s*[-–—]{2,}\s*\n|\n\n", text, maxsplit=1)
        if len(parts) == 2:
...
def write_file_if_path_provided(content: str, file_path: Optional[str]) -> None:
    """Write content to file if path is provided."""
    if not file_path:
        return
    try:
        from pathlib import Path
...
async def fetch_url_text(url: str, *, timeout: float = 15.0, max_bytes: int = 2_000_000) -> dict:
    """Fetch a URL and extract readable text and a best-effort page title.

    - Simplified: no PDF parsing; avoid HEAD preflight to reduce failures.
    - For HTML/text: extract text and title; for other content-types, return empty text.

    Returns dict: ok, status, final_url, content_type, text, page_title, error.
    """
    try:
        async with aiohttp.ClientSession(raise_for_status=False) as session:
            async with session.get(url, allow_redirects=True, timeout=timeout, headers={"User-Agent": "ODR-Ghostwriter/1.0"}) as r:
                status = r.status
                final_url = str(r.url)
        ...
        if status >= 400:
            return {"ok": False, "status": status, "final_url": final_url, "content_type": content_type, "text": None, "page_title": None, "error": f"HTTP {status}"}

        text: Optional[str] = None
        ctype_l = (content_type or "").lower()
        # HTML or unknown: treat as HTML for extraction
        if "html" in ctype_l or final_url.lower().endswith((".html", "/")) or not ctype_l:
            ...
            try:
                ...
                # Prefer first h1 if reasonably present
                h1 = soup.find("h1")
                if h1 and h1.get_text(strip=True):
                    h1_text = h1.get_text(strip=True)
                    if not page_title or len(page_title) < 5:
                        page_title = h1_text[:300]
                text = extract_text_from_html(html)
            except Exception:
                text = extract_text_from_html(html)
        elif ctype_l.startswith("text/"):
            # Generic text
            try:
            ...
            # Binary or other content types (e.g., application/pdf): we do not parse
            text = ""

        # Note: we consider OK even if text is empty for non-HTML content types
        if text is None:
            return {"ok": False, "status": status, "final_url": final_url, "content_type": content_type, "text": None, "page_title": page_title, "error": "Empty or unparseable content"}

        if len(text) > 200_000:
            text = text[:200_000]

        return {"ok": True, "status": status, "final_url": final_url, "content_type": content_type, "text": text, "page_title": page_title, "error": None}
    except Exception as e:
        return {"ok": False, "status": None, "final_url": None, "content_type": None, "text": None, "page_title": None, "error": str(e)}


def extract_text_from_html(html: str) -> str:
    """Extract visible text from HTML with BeautifulSoup."""
    try:
